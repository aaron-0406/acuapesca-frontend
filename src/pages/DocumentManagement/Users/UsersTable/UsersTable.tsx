...
import UserModalLog from '../UserModalLog'
...
export const UsersTable = ({ changeData, setChangeData, updateData }: IProcessesTableProps) => {
    ...
    },
    {
      ...
      render: (data) => (
        <Container display="flex" justifyContent="space-around">
          ...
          />
          <StyledButtonViewLog
            onClick={() => onVisibleModalWithIDLog(data.id)}
            icon={<Icon remixiconClass="ri-eye-line" />}
          />
          <StyledButtonMore
  ...
  const [visibleModalLog, setVisibleModalLog] = useState<boolean>(false)
  ...
  const onVisibleModalWithIDLog = (id: number) => {
    setIdUserSelected(id)
    onToggleModalLog()
  }

  const onToggleModalLog = () => {
    setVisibleModalLog(!visibleModalLog)
  }
  ...
  return (
    <StyledContainer width="100%">
      ...
      />
      <UserModalLog visible={visibleModalLog} setVisible={onToggleModalLog} idUser={idUserSelected} />
    </StyledContainer>
  )
}
...
const StyledButtonViewLog = styled(Button)`
  ${({ theme }) => css`
    background-color: ${theme.colors['$color-info']};
  `}
`
