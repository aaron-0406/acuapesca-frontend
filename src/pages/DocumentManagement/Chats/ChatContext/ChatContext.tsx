...
export const ChatProvider = ({ children }: { children: JSX.Element }) => {
  ...
  const getMessage = (message: IMessage) => {
    if ((contact.id === message.id_emisor && user.id === message.id_receptor) || (contact.id === message.id_receptor && user.id === message.id_emisor)) {
      const { messages } = contact;
    ...
    setContacts(
      contacts.map((contactItem: IContact) => {
        if ((contactItem.id === message.id_emisor && user.id === message.id_receptor) || (contactItem.id === message.id_receptor && user.id === message.id_emisor)) {
    ...
    console.log("Enviando recepcion");
