...
export const Contact = ({ contactProp, contactTypingState }: Props) => {
  ...
  return (
    ...
    >
      ...
      <Container width="100%" maxWidth={'500px'} display="flex" justifyContent="flex-start" flexDirection="column">
        <Text level={3}>{contactProp.fullname}</Text>
        {contactTypingState && (
          <StyledCTyping level={3}>
            {contactTypingState.typing
              ? 'Escribiendo...'
              : contactProp.messages.length > 0
              ? contactProp.messages[contactProp.messages.length - 1].text
              : ''}
          </StyledCTyping>
        )}
      </Container>
